...
public final class GcsSinkConfig extends AivenCommonConfig {
  ...
  public static final String FORMAT_OUTPUT_FIELDS_VALUE_ENCODING_CONFIG =
      "format.output.fields.value.encoding";

  private static final String GROUP_GCS_RETRY_BACKOFF_POLICY = "GCS retry backoff policy";

  public static final String GCS_RETRY_BACKOFF_INITIAL_DELAY_MS_CONFIG =
      "gcs.retry.backoff.initial.delay.ms";

  public static final String GCS_RETRY_BACKOFF_MAX_DELAY_MS_CONFIG =
      "gcs.retry.backoff.max.delay.ms";

  public static final String GCS_RETRY_BACKOFF_DELAY_MULTIPLIER_CONFIG =
      "gcs.retry.backoff.delay.multiplier";

  public static final String GCS_RETRY_BACKOFF_TOTAL_TIMEOUT_MS_CONFIG =
      "gcs.retry.backoff.total.timeout.ms";

  public static final String GCS_RETRY_BACKOFF_MAX_ATTEMPTS_CONFIG =
          ...
          public void ensureValid(final String name, final Object value) {
            ...
            if (longValue < 0) {
              ...
              throw new ConfigException(
                  name, value, "Value must be no more than " + TOTAL_TIME_MAX + " (24 hours)");
            }
          }
        },
        ConfigDef.Importance.MEDIUM,
  ...
  private static void addFileConfigGroup(final ConfigDef configDef) {
        ...
        new ConfigDef.Validator() {
          ...
          public void ensureValid(final String name, final Object value) {
            ...
            if (valueStr.length() > 1024) {
              throw new ConfigException(
                  GCS_BUCKET_NAME_CONFIG, value, "cannot be longer than 1024 characters");
            }
            if (valueStr.startsWith(".well-known/acme-challenge")) {
              throw new ConfigException(
                  GCS_BUCKET_NAME_CONFIG, value, "cannot start with '.well-known/acme-challenge'");
            }
          }
        },
        ConfigDef.Importance.MEDIUM,
    ...
    final String supportedCompressionTypes =
        CompressionType.names().stream().map(f -> "'" + f + "'").collect(Collectors.joining(", "));
    configDef.define(
    ...
    configDef.define(
        ...
        new ConfigDef.Validator() {
          ...
          public void ensureValid(final String name, final Object value) {
            ...
            if ((Integer) value < 0) {
              throw new ConfigException(
                  FILE_MAX_RECORDS, value, "must be a non-negative integer number");
            }
          }
        },
        ConfigDef.Importance.MEDIUM,
  ...
  static Map<String, String> handleDeprecatedYyyyUppercase(final Map<String, String> properties) {
    if (properties.containsKey(FILE_NAME_TEMPLATE_CONFIG)) {
      ...
      final var unitYyyyPattern =
          Pattern.compile("\\{\\{\\s*timestamp\\s*:\\s*unit\\s*=\\s*YYYY\\s*}}");
      template =
          unitYyyyPattern
              .matcher(template)
  ...
  private void validate() {
    ...
    if (RecordGrouperFactory.KEY_RECORD.equals(
        RecordGrouperFactory.resolveRecordGrouperType(filenameTemplate))) {
      if (getMaxRecordsPerFile() > 1) {
        final String msg =
            String.format(
                "When %s is %s, %s must be either 1 or not set",

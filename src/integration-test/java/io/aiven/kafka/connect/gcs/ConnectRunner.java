...
final class ConnectRunner {
  ...
  public ConnectRunner(
      final File pluginDir, final String bootstrapServers, final int offsetFlushIntervalMs) {
  ...
  void start() {
    ...
    // These don't matter much (each connector sets its own converters), but need to be filled with
    // valid classes.
    ...
    final Worker worker =
        new Worker(workerId, time, plugins, config, new MemoryOffsetBackingStore());
        ...
        new FutureCallback<>(
            new Callback<Herder.Created<ConnectorInfo>>() {
              ...
              public void onCompletion(
                  final Throwable error, final Herder.Created<ConnectorInfo> info) {
                if (error != null) {

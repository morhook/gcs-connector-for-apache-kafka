...
/**
 * This is a property-based test for {@link GcsSinkTask} (grouping records by the key) using <a
 * href="https://jqwik.net/docs/current/user-guide.html">jqwik</a>.
 *
...
final class GcsSinkTaskGroupByKeyPropertiesTest extends PbtBase {
  ...
  final void groupByKey(@ForAll("recordBatches") final List<List<SinkRecord>> recordBatches) {
    ...
    final List<String> expectedFileNames =
        lastRecordPerKey.keySet().stream().map(this::createFilename).collect(Collectors.toList());
    ...
    for (final String key : lastRecordPerKey.keySet()) {
      ...
      final String expectedValueSubstring =
          new String((byte[]) record.value(), StandardCharsets.UTF_8);
      final String expectedLine =
          String.format(
              "%s,%s,%d", expectedKeySubstring, expectedValueSubstring, record.kafkaOffset());

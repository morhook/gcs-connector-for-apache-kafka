...
abstract class PbtBase {
  ...
  final Arbitrary<List<List<SinkRecord>>> recordBatches() {
    final RandomGenerator<String> keyGenerator =
        RandomGenerators.samples(new String[] {"key0", "key1", "key2", "key3", null});
        ...
        random -> {
          ...
          for (int topicIdx = 0; topicIdx < topicCount; topicIdx++) {
            ...
            for (int partitionIdx = 0; partitionIdx < partitionCount; partitionIdx++) {
              ...
              recordBuilders.put(
                  tp, new SinkRecordBuilder(random, keyGenerator, tp.topic(), tp.partition()));
            }
          }
  ...
  Map<String, String> basicTaskProps() {
    ...
    taskProps.put(
        GcsSinkConfig.FILE_NAME_TEMPLATE_CONFIG, "{{topic}}-{{partition}}-{{start_offset}}");

...
public final class BucketAccessor {
  ...
  public final List<String> getBlobNames(final String prefix) {
    ...
    return StreamSupport.stream(
            storage.list(bucketName, blobListOption).iterateAll().spliterator(), false)
        .map(BlobInfo::getName)
        .sorted()
        .collect(Collectors.toList());
  }
  ...
  public final List<List<String>> readAndDecodeLines(
  ...
  private List<List<String>> readAndDecodeLines0(
      final String blobName, final String compression, final int[] fieldsToDecode) {
  ...
  private List<String> decodeRequiredFields(
      final String[] originalFields, final int[] fieldsToDecode) {
  ...
  public Iterable<Header> decodeHeaders(final String s) {
    ...
    for (final String header : headers) {
      ...
      final SchemaAndValue schemaAndValue =
          byteArrayConverter.toConnectHeader("topic0", key, value);
